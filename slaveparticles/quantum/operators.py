...
def gf_lehmann(eig_e, eig_states, d_dag, beta, omega, d=None):
    ...
    basis_create = np.dot(eig_states.T.conj(), d_dag.dot(eig_states))
    if d is None:
        tmat = np.square(np.abs(basis_create))
    else:
        tmat = np.dot(eig_states.T.conj(), d.T.dot(eig_states)) * basis_create
...
def expected_value(operator, eig_values, eig_states, beta):
    ...
    aux = np.einsum('i,ji,ji', np.exp(-beta * eig_values),
                    eig_states.conj(), operator.dot(eig_states))
